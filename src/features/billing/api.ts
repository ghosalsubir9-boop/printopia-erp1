...
export class BillingApiService {
  ...
  public static async saveInvoice(invoice: Partial<GSTInvoice>): Promise<GSTInvoice> {
    ...
    if (invoice.invoiceNumber) {
      const isDup = invoices.some(i => i.invoiceNumber === invoice.invoiceNumber);
      if (isDup) {
        throw new Error(`Duplicate Invoice Number: ${invoice.invoiceNumber} already exists.`);
      }
    }

    // Auto-generate invoice number: INV-2026-000001
    const currentYear = new Date().getFullYear();
    const sameYear = invoices.filter(o => o.invoiceNumber.startsWith(`INV-${currentYear}-`));
    ...
    if (!invoiceNumber) {
      while (true) {
        const trialNumber = `INV-${currentYear}-${String(nextSeq).padStart(6, '0')}`;
        const collision = invoices.some(i => i.invoiceNumber === trialNumber);
        if (!collision) {
